/**
 * ê²½ê¸°???°ì´???œë¦¼ (data.gg.go.kr) API ?œë²„?¬ì´???¼ìš°?? * ?˜ì› ?¸í—ˆê°€ ?ì„¸ ?„í™© ?°ì´??ì¡°íšŒ ë°??™ê¸°?? */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[GG Clinic API] ?”ì²­: pIndex=${pIndex}, pSize=${pSize}, sigunNm=${sigunNm || '?„ì²´'}, sync=${sync}`);
    ...
    const leads = rows.map((row: any) => {
      ...
      return {
        ...
        medical_subject: row.TREAT_SBJECT_CONT_INFO || '?˜ì›',
        service_name: row.BIZCOND_DIV_NM_INFO || '?˜ì›',
        category: 'HEALTH',
        latitude: lat || null,
        longitude: lng || null,
        ...
        status: 'NEW',
        operating_status: row.BSN_STATE_NM === '?•ìƒ' ? '?ì—…ì¤? : '?ì—…/?´ì—…',
    ...
    // DB ?™ê¸°??    if (sync && leads.length > 0) {
      ...
      if (dbError) {
        console.error('[GG Clinic API] DB ?€???¤ë¥˜:', dbError);
        return NextResponse.json({
          success: false,
    ...
    console.error('[GG Clinic API] ?¤ë¥˜:', error);
    return NextResponse.json(
