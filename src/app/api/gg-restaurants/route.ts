/**
 * ê²½ê¸°???°ì´???œë¦¼ (data.gg.go.kr) API ?œë²„?¬ì´???¼ìš°?? * ?¼ë°˜?Œì‹???¸í—ˆê°€ ë°??ì—…?„í™© ?°ì´??ì¡°íšŒ ë°??™ê¸°?? */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[GG Rest API] ?”ì²­: pIndex=${pIndex}, pSize=${pSize}, sigunNm=${sigunNm || '?„ì²´'}, sync=${sync}`);
    ...
    const leads = rows.map((row: any) => {
      ...
      return {
        ...
        phone: '', // API?ì„œ ?•ì¸ ?„ìš”
        medical_subject: row.SANITTN_BIZCOND_NM || '?¼ë°˜?Œì‹??,
        service_name: '?¼ë°˜?Œì‹??,
        category: 'FOOD',
        latitude: lat || null,
        longitude: lng || null,
        ...
        status: 'NEW',
        operating_status: row.BSN_STATE_NM === '?ì—…' ? '?ì—…ì¤? : '?ì—…/?´ì—…',
    ...
    // DB ?™ê¸°??    if (sync && leads.length > 0) {
      ...
      if (dbError) {
        console.error('[GG Rest API] DB ?€???¤ë¥˜:', dbError);
        return NextResponse.json({
          success: false,
    ...
    console.error('[GG Rest API] ?¤ë¥˜:', error);
    return NextResponse.json(
