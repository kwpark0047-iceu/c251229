...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[GG Rest API] 요청: pIndex=${pIndex}, pSize=${pSize}, sigunNm=${sigunNm || '전체'}, sync=${sync}`);
    ...
    const leads = rows.map((row: any) => {
      ...
      return {
        ...
        status: 'NEW',
        operating_status: row.BSN_STATE_NM === '영업' ? '영업중' : '폐업/휴업',
    ...
    if (sync && leads.length > 0) {
      ...
      if (dbError) {
        console.error('[GG Rest API] DB 저장 오류:', dbError);
        return NextResponse.json({
          success: false,
    ...
    console.error('[GG Rest API] 오류:', error);
    return NextResponse.json(
