/**
 * 寃쎄린???곗씠???쒕┝ (data.gg.go.kr) API ?쒕쾭?ъ씠???쇱슦?? * 蹂묒썝 ?곸꽭 ?꾪솴 ?곗씠??議고쉶 諛??숆린?? */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[GG Hospital API] ?붿껌: pIndex=${pIndex}, pSize=${pSize}, sigunNm=${sigunNm || '?꾩껜'}, sync=${sync}`);
    ...
    const leads = rows.map((row: any) => {
      ...
      return {
        ...
        medical_subject: row.TREAT_SBJECT_CONT_INFO || '蹂묒썝',
        service_name: row.BIZCOND_DIV_NM_INFO || '蹂묒썝',
        category: 'HEALTH',
        latitude: lat || null,
        longitude: lng || null,
        ...
        status: 'NEW',
        operating_status: row.BSN_STATE_NM === '?곸뾽以? ? '?곸뾽以? : '?먯뾽/?댁뾽',
    ...
    // DB ?숆린??    if (sync && leads.length > 0) {
      ...
      if (dbError) {
        console.error('[GG Hospital API] DB ????ㅻ쪟:', dbError);
        return NextResponse.json({
          success: false,
    ...
    console.error('[GG Hospital API] ?ㅻ쪟:', error);
    return NextResponse.json(
