...
/**
 * 경기도 일반음식점 인허가 데이터 동기화 스크립트
 * 사용법: node scripts/sync-gg-restaurants.js
 */
...
const ggApiKey = process.env.GG_REST_API_KEY || 'bb160ecd612540d38e55c6d3d5da7675';
...
async function syncRestaurants() {
  console.log('🚀 경기도 음식점 데이터 동기화 시작...');
  ...
  while (hasMore) {
    try {
      ...
      const response = await axios.get('https://openapi.gg.go.kr/Genrestrtjpnfood', {
        params: {
          KEY: ggApiKey,
          Type: 'json',
          pIndex: pIndex,
      ...
      if (!response.data.Genrestrtjpnfood) {
        console.log('✅ 모든 데이터를 처리했습니다.');
        break;
      }

      const rows = response.data.Genrestrtjpnfood[1].row;
      if (!rows || rows.length === 0) break;

      const leads = rows
        .filter(row => row.BSN_STATE_NM === '영업')
        .map(row => {
          ...
          return {
            ...
            phone: '', 
            medical_subject: row.SANITTN_BIZCOND_NM || '일반음식점',
            service_name: '일반음식점',
            category: 'FOOD',
            latitude: lat || null,
            longitude: lng || null,
  ...
  console.log(`\n🎉 동기화 완료! 총 ${totalProcessed}건의 음식점 데이터를 처리했습니다.`);
}
