...
/**
 * 경기도 병원 상세 현황 데이터 동기화 스크립트
 * 사용법: node scripts/sync-gg-hospitals.js
 */
...
const ggApiKey = process.env.GG_HOSPITAL_API_KEY || '6e968a5fcec449f683c5c0fcd075802d';
...
// 경기도 주요 지하철역 좌표 데이터 (임시 - src/lib/constants.ts의 데이터와 동기화 권장)
const GG_STATIONS = [
...
async function syncHospitals() {
  ...
  while (hasMore) {
    try {
      ...
      const response = await axios.get('https://openapi.gg.go.kr/GgHosptlM', {
        params: {
          KEY: ggApiKey,
          Type: 'json',
          pIndex: pIndex,
  ...
  console.log(`\n🎉 동기화 완료! 총 ${totalProcessed}건의 병원 데이터를 처리했습니다.`);
}
